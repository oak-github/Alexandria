...
function switchStatus(id){
    let CurrentStatus = findObjectById(id).status
    if (CurrentStatus === true) {
        findObjectById(id).status = false
        document.getElementById(id).style.borderLeftColor = "lightgreen"
    } else
    {
        findObjectById(id).status = true
        document.getElementById(id).style.borderLeftColor = "red"
    }
}

newBook = new book("Harry Potter", "J.K", 149, false)
